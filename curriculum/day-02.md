...
> *English summary: We flash yesterday's `day01_blink` project into the board, see the LED blink and `tick` print over UART, and learn the GPIO push-pull output, UART async framing, ST-Link flashing, and serial-terminal debugging loop.*

> **学习成果 | Learning Outcomes**
> - **实现** (Implement) 把 `day01_blink` 工程烧进板子，看到 LED 闪烁 + 串口打印 `tick`
> - **解释** (Explain) GPIO 输出原理：推挽 vs 开漏，为什么点 LED 默认选推挽
> - **解释** (Explain) UART 异步通信：波特率、起始位、数据位、停止位，为什么 `printf` 能走串口
...
「点灯 + 串口打印」是嵌入式的「Hello World」。看起来简单，但它一次性打通了**代码 → 编译 → 烧录 → 运行 → 观察**这条完整链路。这条链路通了，后面 8 天你加任何模块，都是在已经会跑的骨架上叠功能，而不是每次都从「板子怎么不听话」开始。

**日常类比**: 串口 printf 就像给程序装了个「对讲机」——程序跑到哪、变量是多少、卡在哪一步，它都能实时喊给你听。没有对讲机，程序出了问题你只能两眼一抹黑干瞪眼。今天把这个对讲机配好，后面调试 SHT31、OLED、ESP8266 全靠它。而「波特率必须两边一致」，就像对讲机必须调到同一频道——你调 115200、我调 9600，谁也听不懂谁，收到的全是噪音。
...
> 先自己想答案，再对照参考答案。答得出才算真懂，背下来不算。

**Q1（设计权衡题）**: 点亮板载 LED，CubeMX 给了推挽 (Push-Pull) 和开漏 (Open-Drain) 两个输出模式。这个 LED 该选哪个而不是另一个？从驱动能力、所需外部元件、是否需要电平转换三个角度分析。

> **参考答案**: 选**推挽**。(1) **驱动能力**——推挽既能主动拉高也能主动拉低，驱动能力强；开漏只能拉低，拉高靠外部上拉电阻，驱动弱。(2) **外部元件**——推挽不需要任何额外电阻（板载 LED 已有限流电阻）；开漏必须外接上拉才能输出高电平，多一个元件。(3) **电平转换**——开漏的用武之地是 I2C 总线（多设备线与）和电平转换（用上拉接到不同电压），点 LED 既不共享总线也不跨电平，用开漏是杀鸡用牛刀还添麻烦。一句话：**普通 GPIO 输出默认推挽，I2C 才用开漏**（Day 3 会用到）。

**Q2（原理追问题）**: 串口配置写成「115200 8N1」，这四个字段各代表什么？为什么串口助手和单片机两边这四个字段必须完全一致，否则收到的就是乱码？

> **参考答案**: 115200 = **波特率** (每秒传 115200 个 bit)；8 = **数据位** (一帧带 8 bit 数据，即一个字节)；N = **校验位 None** (无奇偶校验)；1 = **停止位** (1 个高电平停止位)。UART 是**异步通信，没有时钟线**，双方靠「约定同样的波特率」各自用本地时钟按节奏采样。如果一边 115200、另一边 9600，接收方的采样节奏和发送方完全错位，采到的每一位都错位，解出来的字节就是乱码。同理数据位/校验/停止位不一致，帧边界对不齐也会错。所以串口调试 90% 的「乱码」问题，根因都是这四个字段没对齐。

**Q3（故障分析题）**: 烧录成功（CubeIDE 显示下载完成），但 LED 不闪、串口也没收到任何字符。按可能性从高到低列出至少 3 个原因及排查动作。

> **参考答案**: (1) **BOOT0 拉高了**（板子处于 ISP 烧录模式而不是正常运行模式，程序烧进去了但不执行）→ 检查 BOOT0 跳线 = 0，复位。(2) **LED 引脚标签和实际板载 LED 对不上**（Day 1 给 PA5 起名 `LED`，但你板子的灯可能在 PB5）→ 查板子原理图/丝印，改 CubeMX 引脚重新 Generate。(3) **程序停在断点 / Debug 模式没按 F8 继续**→ 退出 Debug 视图，或按 F8 Resume 全速运行。(4) **串口没重定向 `__io_putchar` 或 USART1 没初始化**→ 回 Day 1 §5.4 检查代码块在 USER CODE 区内、`MX_USART1_UART_Init()` 被调用。(5) **串口助手 COM 口选错 / 波特率不对**→ 确认设备管理器里的 COM 号、改 115200。排查顺序：先看 BOOT0 → 再确认 LED 引脚 → 再查 Debug 是否卡住 → 最后查串口配置。

### 今日评分标准 | Daily Rubric

| 维度 | 满分 | 评分细则 |
|------|------|---------|
| 完成度 | 4 | LED 闪烁 + 串口稳定收到 `tick` + 至少完成作业 1-2 |
| 理解深度 (CC 回答) | 3 | 能答出 Q1 推挽/开漏选择 + Q2 波特率必须一致的理由 |
| 代码/接线质量 | 2 | 代码在 USER CODE 区、串口配置 115200 8N1、断点调试会用 |
| 创意拓展 | 1 | 完成 SOS 摩斯码 或 串口回显，或加了 `log()` 时间戳 |

---

> **今日产出 = 明日输入**: 点灯 + printf 跑通的 `day01_blink` 工程（串口是你的「眼睛」）= Day 3 接 SHT31 传感器、用串口打印真实温湿度数值的输入。今天串口看不到东西，明天传感器读数就无处打印。
