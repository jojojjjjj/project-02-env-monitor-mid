# Day 1: 项目导览 + 认识 STM32L433CCT6 (KE1 板) + 工具链安装 | Project Tour + Meet the STM32L433CCT6 (KE1 Board) + Toolchain

> **⏱ 预计耗时 | Estimated Time**: 约 5-6 小时（工具链下载安装占大头，看网速） · ~5-6 hours (toolchain download dominates)

> *English summary: Today we set up the STM32CubeIDE toolchain, identify the STM32L433CCT6 + ESP8266 two-chip architecture, and create a compilable empty project — no code is run on the board yet (flashing starts Day 2).*

> **学习成果 | Learning Outcomes**
> - **识别** (Identify) 我们要复刻的「WiFi 温湿度网络时钟」由哪些模块组成、各自职责
> - **解释** (Explain) 为什么主控 STM32L433CCT6 必须配 ESP8266 协处理器，而不是用一片带 WiFi 的芯片
> - **解释** (Explain) 「复刻 (replication)」学习模式：代码已提供，你负责接线 + 烧录 + 配置 + 调试
> - **实现** (Implement) 在自己电脑上装好 STM32CubeIDE，新建一个能编译通过的 `day01_blink` 工程
>
> **产出 | Deliverable**: 一台能被电脑识别出 COM 口的 KE1 板 + 一个能在 CubeIDE 里编译通过的空工程

## 为什么学这个 | Why This Matters

工具链是嵌入式开发的「地基」。盖楼不先打地基，上面写得再漂亮也会塌。今天不写一行逻辑代码、不让板子做任何花哨的事——只把「编辑器 + 编译器 + 烧录器 + 串口」这条链路打通。

**日常类比**: 工具链装不好，就像你想做木工却发现工作台没装好、锯子没磨、量尺刻度看不清——你花一天调好台子，后面 9 天每一刀都又快又准；图省事跳过，后面天天返工。STM32L433 和 ESP8266 是两块独立芯片这件事，就像**电脑主机和 USB 无线网卡**的关系——主机算力强但没有联网能力，要靠一张网卡（ESP8266）通过 USB（这里换成 UART）帮它上网。
...
- Flash：256KB，RAM：64KB（对我们这个项目绰绰有余）—— 注意要选 **CCT6**（256KB Flash），不是 CBT6（128KB）；CBT6 虽引脚兼容但 Flash 减半，固件可能放不下
- 封装：LQFP48（KE1 板上的 **STM32L433CCT6** 是 48 脚；固件 `.ioc` 里 `Mcu.UserName=STM32L433CCTx` 对应的就是它）
...
> **Why CCT6 not CBT6?** 同为 LQFP48 封装，CCT6 是 256KB Flash、CBT6 是 128KB Flash。本项目固件含 WiFi AT 命令解析 + OLED 字库 + SNTP，128KB 可能不够。BOM 上就是 CCT6，新建工程选 `STM32L433CCTx`。

### 3.2 KE1 板是什么？

KE1 是 UP主「乐在程上」为教学设计的 STM32L433 开发板，板载：
- STM32L433CCT6 主控（256KB Flash）
...
3. 搜 `L433`，选 **`STM32L433CCTx`**（KE1 板的 STM32L433CCT6，256KB Flash，LQFP48；**不要选成 `CBTx`**，那是 128KB 的 CBT6，固件可能放不下），点 Next。
...
> 本节是今日「为什么」类问题的核心。先自己想答案，再对照参考答案——能答出来才算真懂。

**Q1（设计权衡题）**: 为什么这个项目用「STM32L433 + 外挂 ESP8266」两块芯片，而不是直接用一片带 WiFi 的 ESP32？从复刻还原度、架构学习价值、功耗三个角度分析。

> **参考答案**: (1) **复刻还原度**——UP主视频原设计就是 L433+ESP8266，复刻要对齐引脚/时钟/AT 命令时序，换芯片就跑不通原固件。(2) **架构学习价值**——「主控大脑 + 网卡协处理器」是嵌入式最经典的分工（电脑主机+网卡、手机 SoC+WiFi 模组都是这个模式），一片 ESP32 把所有功能揉在一起反而学不到「两芯片用 UART+AT 协议通信」这套真实工程能力。(3) **功耗**——L433 是低功耗系列，需要联网时唤醒 ESP8266，不需要时 ESP8266 可休眠，比 ESP32 常开更省电。代价是硬件复杂度高一档、占用 UART 口。

**Q2（原理追问题）**: 在 §5.2 里我们把系统时钟配成 32MHz 而不是 L433 支持的最高 80MHz。为什么 32MHz 对这个项目反而更合适？

> **参考答案**: (1) 这个项目算力需求极低（读传感器、刷 OLED、发 AT 命令），32MHz 远远够用，提不到 80MHz 的性能收益。(2) 功耗随主频近似线性上升，32MHz 更省电、发热更小。(3) 串口波特率分频在 32MHz 下更整（115200 能整除得到 0 误差），80MHz 下某些波特率会有分频余差导致收发漂移。(4) UP主 固件就是 32MHz，复刻必须对齐，否则时序敏感的代码（AT 命令超时、I2C 时序）可能错。一句话：够用、省电、分频整、对齐参考源码。

**Q3（故障分析题）**: 你装好 CubeIDE、插上 KE1 板，但设备管理器里完全看不到 COM 口。按可能性从高到低列出至少 3 个原因及对应排查动作。

> **参考答案**: (1) **USB 线是「充电线」不能传数据**（最常见，~50% 概率）→ 换一根确定能传数据的线（比如能连手机的线）。(2) **ST-LINK 驱动没装**（安装 CubeIDE 时跳过了驱动勾选）→ 重装 CubeIDE 时务必勾上 ST-LINK 驱动，或单独装 ST-LINK USB driver。(3) **USB 口供电不足 / 接的是 USB Hub**→ 直接插主板 USB 口，避开 Hub/前面板。(4) **板子没上电 / 拨码开关没拨到供电档**→ 看板子电源指示灯是否亮。(5) 驱动冲突（设备管理器有黄色感叹号）→ 右键卸载设备，拔插重装。排查顺序：换线 → 换口 → 查驱动 → 看灯。

### 今日评分标准 | Daily Rubric

| 维度 | 满分 | 评分细则 |
|------|------|---------|
| 完成度 | 4 | COM 口能识别 + `day01_blink` 工程 0 error 编译通过 |
| 理解深度 (CC 回答) | 3 | 能答出 Q1 两芯片分工 + Q2 时钟选择的至少 2 条理由 |
| 代码/接线质量 | 2 | 工程结构规范，USER CODE 区使用正确，时钟/GPIO/USART 配置无误 |
| 创意拓展 | 1 | 主动探索：画了架构图、或试了不同时钟配置、或读了参考源码 |

---

> **今日产出 = 明日输入**: `day01_blink` 工程（0 error，含 LED + printf 重定向代码）= Day 2 烧录 + 点灯 + 串口看日志的输入。今天编译不过，明天就没东西可烧。
