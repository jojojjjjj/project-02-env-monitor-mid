...
读完本指南并完成一次完整排错后，你应当能够：

- **识别 (Identify)** 项目 10 类常见硬件故障的现象特征（串口无输出 / I2C 扫不到 / AT 无响应 / WiFi 失败 / SNTP 不准 等）
- **解释 (Explain)** 为什么「一次只改一处」、为什么「80% 玄学问题根因是供电」、为什么 ESP-01S 要加滤波电容
- **实现 (Implement)** 按诊断总流程图逐级排查，用串口 printf + 万用表定位到具体故障点并修复
- **分析 (Analyze)** 一个未知现象时，能把它归入流程图的某一节点，按「症状→原因→对策」三段式推导

## 为什么学这个 | Why This Matters

> 类比：**排错像医生问诊，不像猜谜。** 医生不会一次性开全套检查，而是先问「哪里不舒服」→ 再缩小范围 → 再针对性检查。嵌入式排错同理：先看串口有没有 printf（判断 MCU 活没活）→ 再看 SHT31 读没读到（I2C 通不通）→ 再看 ESP-01S 回不回 OK（UART 通不通）……一级一级往下。**一次只改一处、改完就测**，是工程师和爱好者的分水岭。卡住时去 `verify/project02/inspect/` 对照 KE1 源码看原始实现，往往一眼就看出自己哪接错了。
...
> 这些是故障分析题——给你现象，让你按「症状→原因→对策」推导。参考答案见上文对应小节。

**Q1: 插上 ESP-01S 后 STM32 反复重启，OLED 闪烁，连 WiFi 瞬间死机。按可能性排序，最可能的根因是什么？如何验证？**

> **参考答案**: 最可能根因是 **ESP-01S 供电不足**（占此类现象 ~80%）。ESP-01S 发射 WiFi 时瞬态电流 200-300mA，把 3.3V 拉低，STM32 欠压复位。验证方法：(1) 用万用表量 ESP-01S VCC 在 WiFi 连接瞬间是否跌到 3.0V 以下；(2) 先拔掉 ESP-01S，看 STM32 是否不再重启——若拔了就好，基本锁定供电。对策：VCC 并 100μF 电解 + 0.1μF 陶瓷电容；或 ESP-01S 单独用一路 AMS1117-3.3 从 USB 5V 稳压，只共 GND。次要可能：USB 口/线供电不足、面包板接触电阻大。

**Q2: 串口能收到 STM32 的 printf 输出，但发 `AT` 给 ESP-01S 收到乱码或无响应。按流程图应该先查哪一级？最可能原因是什么？**

> **参考答案**: 串口有 printf 说明 [1] 串口驱动/CH340 OK、MCU 活着。问题在 [5] ESP8266 AT 无响应这一级。最可能原因（按概率）：(1) **波特率不匹配**——ESP-01S 出厂 115200，但老版 ESP-01 是 9600，或被改过；先用 115200 测，乱码就改 9600 测；(2) **TX/RX 接反**——没按「T 接 R」交叉，TX 接了 TX；(3) CH_PD 没拉高；(4) 供电不足。排查顺序：先确认接线交叉对、CH_PD 接 3V3，再试波特率，最后用 USB-TTL 直连 ESP-01S 发 `AT` 排除 MCU 侧问题。

**Q3: 为什么排错要「一次只改一处、改完就测」，而不是「一次改三处省时间」？**

> **参考答案**: 一次改多处，如果问题解决了，你**不知道是哪一处起的作用**——下次同样问题还得从头试三处，没积累经验；如果没解决，也无法缩小范围。一次只改一处 + 改完就测，是把「黑箱」逐变量拆开，每次只动一个变量，能精确归因。这是科学实验的「控制变量法」在工程排错中的应用，也是工程师和「瞎试型」爱好者的核心区别。代价是慢一点，但每次排错都转化为可复用的经验。

---

## 评分标准 | Rubric (排错环节，过程性评价)

| 维度 | 满分 | 评分细则 |
|------|------|---------|
| 完成度 | 4 | 能独立按诊断流程图把一个故障排查到根因并修复（不留「不知道怎么好的」）|
| 理解深度 (CC 回答) | 3 | 能答对 Q1-Q3 中至少 2 个，说清供电根因概率、AT 排查顺序、控制变量法 |
| 排错质量 | 2 | 留下排错记录（现象/试过什么/结果/根因），串口日志完整 |
| 创意拓展 | 1 | 总结出一条自己的「避坑清单」、或帮同学排错并讲清原理 |
